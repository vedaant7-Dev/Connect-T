...
var STORAGE_KEY = "janseva_jobs_listings_v4";
function generateId() {
...
function JobsProvider(_a) {
        ...
        async_storage_1.default.getItem(STORAGE_KEY).then(function (raw) {
            if (raw) {
                try {
                    ...
                    var migrated = parsed
                        .map(function (j) { return (__assign(__assign({}, j), { hired: j.hired || [], shortlisted: j.shortlisted || [], rejected: j.rejected || [] })); })
                        .filter(function (j) { return typeof j.employerId === "string" && j.employerId.trim() !== ""; });
                    setJobs(migrated);
                    return;
                }
                catch (_a) { }
    ...
    var addJob = function (data) {
        var job = __assign(__assign({}, data), { id: generateId(), applicants: [], messages: [], hired: [], shortlisted: [], rejected: [], active: true, createdAt: new Date().toISOString() });
        save(__spreadArray([job], jobs, true));
    };
    var addJobMessage = function (jobId, message) {
        save(jobs.map(function (j) { return (j.id === jobId ? __assign(__assign({}, j), { messages: __spreadArray(__spreadArray([], (j.messages || []), true), [message], false) }) : j); }));
    };
    ...
    return (<JobsContext.Provider value={{ jobs: jobs, loading: loading, addJob: addJob, addJobMessage: addJobMessage, applyJob: applyJob, hasApplied: hasApplied, getJobsByEmployer: getJobsByEmployer, toggleJobActive: toggleJobActive, shortlistApplicant: shortlistApplicant, rejectApplicant: rejectApplicant, deleteJob: deleteJob }}>
      {children}
