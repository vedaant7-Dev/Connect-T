...
// ─── Seeker job card ─────────────────────────────────────────────────────────
function JobCard(_a) {
    var job = _a.job, onApply = _a.onApply, applied = _a.applied, near = _a.near, onOpen = _a.onOpen;
    ...
    return (<react_native_1.View style={s.card}>
      ...
      <react_native_1.TouchableOpacity activeOpacity={0.75} onPress={onOpen} style={s.cardTappable}>
        <react_native_1.View style={s.cardHeader}>
          ...
          <vector_icons_1.Feather name="chevron-right" size={16} color="#94A3B8"/>
        </react_native_1.View>
...
function EmployerDashboard(_a) {
    ...
    var _b = (0, react_1.useState)(null), deleteTarget = _b[0], setDeleteTarget = _b[1];
    ...
    var notificationCount = myJobs.filter(function (j) { return j.applicants.length > 0; }).length;
    ...
    return (<react_native_1.View>
      <react_native_1.View style={s.dashboardTopRow}>
        <react_native_1.Text style={s.sectionTitle}>Dashboard</react_native_1.Text>
        <react_native_1.TouchableOpacity style={s.notifyIconBtn} activeOpacity={0.8}>
          <vector_icons_1.Feather name="bell" size={18} color="#C2410C"/>
          {notificationCount > 0 && (<react_native_1.View style={s.notifyDot}>
              <react_native_1.Text style={s.notifyDotText}>{notificationCount}</react_native_1.Text>
            </react_native_1.View>)}
        </react_native_1.TouchableOpacity>
      </react_native_1.View>
      {/* KPI grid */}
      ...
      <react_native_1.View style={s.kpiRow}>
        <react_native_1.TouchableOpacity style={[s.kpiSmall, { backgroundColor: "#D1FAE5", borderColor: "#A7F3D0" }]} onPress={function () { return router.push("/jobs/status?status=shortlisted"); }} activeOpacity={0.85}>
        ...
        <react_native_1.TouchableOpacity style={[s.kpiSmall, { backgroundColor: "#FEE2E2", borderColor: "#FECACA" }]} onPress={function () { return router.push("/jobs/status?status=rejected"); }} activeOpacity={0.85}>
        ...
        <react_native_1.TouchableOpacity style={[s.kpiSmall, { backgroundColor: "#EFF6FF", borderColor: "#BFDBFE" }]} onPress={function () { return router.push("/jobs/status?status=pending"); }} activeOpacity={0.85}>
        ...
        <react_native_1.TouchableOpacity style={[s.kpiSmall, { backgroundColor: "#FFF7ED", borderColor: "#FED7AA" }]} onPress={function () { return router.push("/jobs/status?status=active"); }} activeOpacity={0.85}>
      ...
      <react_native_1.TouchableOpacity onPress={onPostJob} activeOpacity={0.85} style={s.postCtaWrap}>
        <expo_linear_gradient_1.LinearGradient colors={["#059669", "#10B981", "#34D399"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.postCta}>
          ...
          <react_native_1.Text style={s.emptySubText}>Tap "Post a New Job" above to get started</react_native_1.Text>
            ...
            return (<react_native_1.View key={job.id} style={s.jobPerfCard}>
              ...
              <react_native_1.View style={{ flexDirection: "row", alignItems: "stretch" }}>
                <react_native_1.TouchableOpacity onPress={function () { return router.push("/jobs/active/".concat(job.id)); }} activeOpacity={0.85} style={[s.viewAppsBtn, { flex: 1, opacity: job.applicants.length === 0 ? 0.5 : 1 }]}>
                  <vector_icons_1.Feather name="users" size={14} color="#EA580C"/>
                  <react_native_1.Text style={s.viewAppsBtnText}>
                    {job.active ? "Open active job" : "View ".concat(job.applicants.length, " applicant").concat(job.applicants.length !== 1 ? "s" : "")}
                  </react_native_1.Text>
                  <vector_icons_1.Feather name="chevron-right" size={14} color="#EA580C"/>
                </react_native_1.TouchableOpacity>
...
function JobsHomeScreen() {
    ...
    var visibleJobs = activeJobs.filter(function (j) { return !jobsUser || jobsUser.role !== "seeker" || !j.applicants.includes(jobsUser.id); });
    ...
    return (<react_native_1.View style={s.root}>
      ...
      <react_native_1.FlatList data={[]} keyExtractor={function () { return ""; }} renderItem={null} contentContainerStyle={[s.list, { paddingBottom: Math.max(insets.bottom, 8) + 80 }]} showsVerticalScrollIndicator={false} ListHeaderComponent={isEmployer ? (<EmployerDashboard jobs={jobs} employerId={jobsUser.id} onPostJob={function () { return router.push("/jobs/(tabs)/post"); }} onToggle={toggleJobActive} onShortlist={shortlistApplicant} onReject={rejectApplicant} onDelete={deleteJob}/>) : (<>
              {nearbyJobs.filter(function (job) { return !jobsUser || !hasApplied(job.id, jobsUser.id); }).length > 0 && (<react_native_1.View style={s.section}>
                  <react_native_1.View style={s.sectionHeader}>
              ...
              {nearbyJobs.filter(function (job) { return !jobsUser || !hasApplied(job.id, jobsUser.id); }).map(function (job) { return (<JobCard key={job.id} job={job} near applied={jobsUser ? hasApplied(job.id, jobsUser.id) : false} onApply={function () { return handleApply(job); }} onOpen={function () { return router.push("/jobs/detail/".concat(job.id)); }}/>); })}
                </react_native_1.View>)}
              <react_native_1.View style={s.section}>
                ...
                {visibleJobs.length === 0 ? (<react_native_1.View style={s.empty}>
                  ...
                  </react_native_1.View>) : (visibleJobs.map(function (job) { return (<JobCard key={job.id} job={job} applied={jobsUser ? hasApplied(job.id, jobsUser.id) : false} onApply={function () { return handleApply(job); }} onOpen={function () { return router.push("/jobs/detail/".concat(job.id)); }}/>); }))}
              </react_native_1.View>
            </>)}/>
